...
def get_chanlun_analysis(
    ...
    level: Annotated[str, "K-line level: daily/weekly/monthly (default daily)"] = "daily",
    start_date: Annotated[str, "Start date YYYY-MM-DD, empty=from config (default 20240101)"] = "",
) -> str:
    """缠论综合分析 — 返回中枢、走势类型、背驰、买卖点的完整文本报告。

    基于「缠中说禅」理论，对A股标的进行缠论分析（支持日/周/月线级别）。
    分析内容：K线包含处理→分型识别→笔划分→线段→中枢→走势类型→背驰→买卖点。

    使用场景：当传统技术指标信号模糊或需额外验证时，缠论提供基于几何递归的独立分析视角。
    注意：缠论分析需要本地缓存中有充足的K线数据。如返回"数据不足"提示，请先在数据同步页面同步数据。
    """

...
def create_market_analyst(llm):
    ...
    def market_analyst_node(state):
     ...
     📈 缠论分析（**必须调用**，A 股技术分析核心组件）：
     - **必须调用 get_chanlun_analysis 获取缠论（缠中说禅理论）视角的技术分析结果**
     - 缠论核心概念：
       - 走势必完美：任何级别的趋势终将完成
     ...
     - 缠论作为基于几何递归的独立分析框架，与传统指标形成互补验证
     - 缠论买卖点与传统指标信号互为验证，不一致时需仔细分析原因
     - 注意：缠论分析需要本地缓存中有充足的日线数据，若返回数据不足提示，标注 [数据缺失: 缠论分析]，不影响传统指标分析
    """
...
3. **必须调用 get_chanlun_analysis** 获取缠论分析结果，作为技术分析的结构验证
4. 撰写详细的技术分析报告，包含具体数值和可操作的交易建议
